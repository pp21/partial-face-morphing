...
def getpotins_sw(img):
    ...
    # get nose points
    eyesnosepoints = points[17:27]
    eyesnosepoints.append( [ round( ( points[16][0]+points[45][0] )/2 ), round( ( points[16][1]+points[45][1] )/2 ) ] )
    eyesnosepoints.append( [ round( ( points[15][0]+points[46][0] )/2 ), round( ( points[15][1]+points[46][1] )/2 ) ] )
    eyesnosepoints.append( [ round( ( points[29][0]+points[47][0] )/2 ), round( ( points[29][1]+points[47][1] )/2 ) ] )
    eyesnosepoints.append( [ points[54][0], points[30][1] ] )
    eyesnosepoints.append( [ round( ( points[35][0]+points[54][0] )/2 ), round( ( points[35][1]+points[54][1] )/2 ) ] )
    eyesnosepoints.append( [ round( ( points[33][0]+points[51][0] )/2 ), round( ( points[33][1]+points[51][1] )/2 ) ] )
    eyesnosepoints.append( [ round( ( points[31][0]+points[48][0] )/2 ), round( ( points[31][1]+points[48][1] )/2 ) ] )
    eyesnosepoints.append( [ points[48][0], points[30][1] ] )
    eyesnosepoints.append( [ round( ( points[29][0]+points[40][0] )/2 ), round( ( points[29][1]+points[40][1] )/2 ) ] )
    eyesnosepoints.append( [ round( ( points[1][0]+points[41][0] )/2 ), round( ( points[1][1]+points[41][1] )/2 ) ] )
    eyesnosepoints.append( [ round( ( points[0][0]+points[36][0] )/2 ), round( ( points[0][1]+points[36][1] )/2 ) ] )
        
    bb = min( points[49][1], points[51][1], points[53][1] )
      
    return eyesnosepoints, bb
...
def facedswap(img1, img2):       
    ...
    # adjust source component size
    sl = min(points2)[0]
    sr = max(points2)[0]
    st = min(points2, key=lambda x:x[1])[1]
    sb = max(points2, key=lambda x:x[1])[1]
    tw = tr - tl
    th = tb - tt
    sw = sr - sl
    sh = sb - st
    sbpidx = [13, 14, 15, 16, 17]
    if th > sh:
        for sbp in sbpidx:
            points2[sbp][1] += round( (th-sh)/2 )
            points2[sbp][1] = min(points2[sbp][1], bb2)
    slpidx = [16, 17]
    srpidx = [13, 14]
    if tw > sw:
        for slp in slpidx:
            points2[slp][0] -= round( (tw-sw)/2 )
        for srp in srpidx:
            points2[srp][0] += round( (tw-sw)/2 )            
...
def mor_eyes_nose(imgpath1, imgpath2, alpha1, alpha2):
    # Read images
